...
fn matching_engine() {
    ...
    m.limit(new_order(100, 10, Side::Bid));
    m.limit(new_order(200, 5, Side::Ask));
...
fn marketable_order() {
    ...
    m.limit(new_order(100, 10, Side::Bid));
    m.limit(new_order(200, 5, Side::Ask));
    m.limit(new_order(200, 3, Side::Bid));

    m.limit(new_order(100, 2, Side::Ask));
...
fn marketable_order_cross_multiple_limits() {
    ...
    m.limit(new_order(96, 4, Side::Bid));
    m.limit(new_order(99, 4, Side::Bid));
    m.limit(new_order(100, 10, Side::Bid));
    m.limit(new_order(200, 5, Side::Ask));
    m.limit(new_order(202, 5, Side::Ask));
    m.limit(new_order(203, 5, Side::Ask));

    m.limit(new_order(99, 3, Side::Ask));
    
    assert_eq!(m.best_limits(), (100, 200));
    assert_eq!(m.size_at_price(100), 7);

    m.limit(new_order(97, 9, Side::Ask));

    assert_eq!(m.best_limits(), (99, 200));
    assert_eq!(m.size_at_price(99), 2);

    m.limit(new_order(203, 12, Side::Bid));
...
fn insert_within_bid_ask_spread() {
    ...
    m.limit(new_order(101, 3, Side::Bid));
    m.limit(new_order(200, 2, Side::Ask));
    ...
    m.limit(new_order(156, 1, Side::Ask));
...
fn consume_and_insert() {
    ...
    m.limit(new_order(100, 10, Side::Bid));
    m.limit(new_order(200, 5, Side::Ask));
    m.limit(new_order(201, 5, Side::Ask));
    m.limit(new_order(203, 5, Side::Ask));

    m.limit(new_order(202, 12, Side::Bid));

    assert_eq!(m.best_limits(), (202, 203));
    assert_eq!(m.size_at_price(202), 2);

    m.limit(new_order(150, 3, Side::Ask));
